...
export const fetchAllProducts = catchAsyncErrors(async (req, res, next) => {
  ...
  //Query for faching new products
  ...
  //Query for faching Top rating products
  const topRatedQuery = `
  SELECT p.*,
  COUNT(r.id) AS review_count
  FROM products p
...
export const postProductReview = catchAsyncErrors(async (req, res, next) => {
  ...
  const { rows } = await database.query(purchasheChecckQuery, [
    req.user.id,
    productId,
  ]);
  ...
  if (rows.length === 0) {
    return res.status(403).json({
      success: false,
      message: "You can only review a product you've purchashed.",
    });
  }
...
export const deleteReview = catchAsyncErrors(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message: "Your reviews has been deleted.",
    review: review.rows[0],
    product: updateProduct.rows[0],
  });
});
